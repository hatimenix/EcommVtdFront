...
export const ContextProvider = ({ children }) => {
  ...
  const fetchData = async (id) => {
    const response = await axios.get(
      `https://api.paiperleckelearning.com/test/?search=${id}`
    );
  ...
  const postdatadetails = async (id) => {
    const postestdetial = await axios.get(`https://api.paiperleckelearning.com/test/${id}`);
  ...
  const putdata = async (formdata) => {
    const edittest = await axios.put("https://api.paiperleckelearning.com/test/", formdata);
    setteditest(edittest.data);
  };

  const fetchDataStat = async () => {
    const stat = await axiosClient.get('/test/');
    setstatdata(stat.data);
  };

  const statquestion = async () => {
    const statquestion = await axios.get(
      "https://api.paiperleckelearning.com/test/total_questions_by_module/"
    );
  ...
  const fetchModule = async (id) => {
    const Module = await axios.get(
      `https://api.paiperleckelearning.com/module/?search=${id}`
    );
    setModule(Module.data);
  };
  const getModule = async (id) => {
    const Module = await axiosClient.get(`/module/`);
    const numberofids=Module.data
    setgetModule(numberofids)
  }
  const getChapitre = async (id) => {
    const chapitre = await axios.get(
      `https://api.paiperleckelearning.com/chapitre/?search=${id}`
    );
    setgetChapitre(chapitre.data);
  };
  const fetchFormatione = async () => {
    const formation = await axios.get(`https://api.paiperleckelearning.com/Formation/`);
    setFormation(formation.data);
  };
  const post = async (formdata) => {
    const postResponse = await axios.post(
      "https://api.paiperleckelearning.com/user/",
      formdata
    );
  ...
  const postresultat = async (formdataresultat) => {
    const post = await axios.post(
      "https://api.paiperleckelearning.com/resultat/",
      formdataresultat
    );
  ...
  const Gettentative = async (idtest) => {
    //const gettentative = await axios.get(`https://api.paiperleckelearning.com/resultat/${idtest}/`)
    const gettentative = await axios.get(`https://api.paiperleckelearning.com/resultat/2/`);
  ...
  const Restart = async (formdata) => {
    ...
    const restarttesntative = await axios.put(
      `https://api.paiperleckelearning.com/resultat/2/`,
      formdata
    );
  ...
  const Postcertificat = async (formdata) => {
    const certificat = await axios.post(
      "https://api.paiperleckelearning.com/certificat/",
      formdata
    );
    setpostcerif(certificat.data);
  };
  const Getcertificat = async () => {
    const getcertificat = await axios.get("https://api.paiperleckelearning.com/certificat/");
